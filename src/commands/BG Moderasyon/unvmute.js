...
module.exports = {
  ...
  execute: async (client, message, args, embed, author, channel, guild) => {
    ...
    if (!member) return message.reply({ embeds: [embed.setDescription(`Geçerli bir kullanıcı belirt.`)] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
    ...
    message.reply({ embeds: [embed.setDescription(`${member} kullanıcısının voice susturulması kaldırıldı.`)] }).catch((err) => console.log(err), client.ytick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
    ...
    kaldırılan Tarih: \`${moment(Date.now()).format("LLL")}\``)] });
  }
}