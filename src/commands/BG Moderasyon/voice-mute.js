...
module.exports = {
  ...
  execute: async (client, message, args, embed, author, channel, guild) => {
    if (!message.member.permissions.has("ADMINISTRATOR") && !message.member.roles.cache.has(config.penals.voicemute.staff)) return message.reply({ embeds: [embed.setDescription(`Komutu kullanabilmek için yetkiniz yetersiz.!`)] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
    ...
    message.reply({ embeds: [embed.setDescription(`${member} kullanıcısı **"${reason}"** sebebiyle **${sure}** boyunca sesli kanallarda susturuldu. \`(Ceza ID: #${db.fetch(`ceza_${guild.id}`)})\``)] }).catch((err) => console.log(err), client.ytick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
    ...
    ${member ? member.toString(): member.username} kullanıcısı seste susturuldu.
    ...
    setTimeout(() => {
      ...
      client.channels.cache.get(config.penals.mute.log).send({ embeds: [embed.setDescription(`${member} kişisinin ${moment(Date.now()).format("LLL")} tarihinde **${reason}** sebebiyle aldığı seste susturulma cezası süresi bittiği için kaldırıldı.`)] })}
    }, ms(sure));
